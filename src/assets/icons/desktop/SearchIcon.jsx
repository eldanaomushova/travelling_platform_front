export const SearchIcon = ({ className, width = "32px", height = "32px" }) => {
    return (
        <svg
            className={className}
            width={width}
            height={height}
            viewBox="0 0 40 40"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
        >
            <path
                d="M32 32L26.2094 26.2094M26.2094 26.2094C27.1999 25.2189 27.9856 24.043 28.5217 22.7488C29.0577 21.4547 29.3336 20.0676 29.3336 18.6668C29.3336 17.266 29.0577 15.879 28.5217 14.5848C27.9856 13.2906 27.1999 12.1147 26.2094 11.1242C25.2189 10.1337 24.043 9.34802 22.7488 8.81196C21.4547 8.27591 20.0676 8 18.6668 8C17.266 8 15.879 8.27591 14.5848 8.81196C13.2906 9.34802 12.1147 10.1337 11.1242 11.1242C9.12382 13.1247 8 15.8378 8 18.6668C8 21.4958 9.12382 24.209 11.1242 26.2094C13.1247 28.2098 15.8378 29.3336 18.6668 29.3336C21.4958 29.3336 24.209 28.2098 26.2094 26.2094Z"
                stroke="black"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
        </svg>
    );
};
